...
async function fetcher(url: string) {
  ...
  if (!contentType.includes('application/json')) {
    ...
    console.error('Non-JSON response from notifications API:', text)

    throw new Error(
      'Notifications API did not return JSON. Check app/api/notifications/route.ts.'
    )
  }
...
function getTypeBadgeClass(type?: string | null) {
  ...
  if (type === 'system') {
    return 'bg-slate-100 text-slate-800 dark:bg-slate-900/70 dark:text-slate-200'
  }
...
export default function AdminNotificationsPage() {
  ...
  const canFetchNotifications = !loading && user?.role === 'admin'
  ...
  if (loading || isLoading) {
    return (
      <AppLoading
        title="Preparing admin notifications"
        subtitle="Loading system notifications, disputes, alerts, user updates, and project messages."
      />
    )
  }
      ...
      <main className="container mx-auto max-w-7xl px-4 py-8">
        <div className="page-hero slide-up">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="page-kicker">Admin Notifications</p>

              <h1 className="page-title">Notification Center</h1>

              <p className="page-subtitle">
                Track system alerts, user activity, dispute updates, project
                warnings, and important admin messages.
              </p>
            </div>
        ...
        <Card className="professional-card">
          <CardHeader>
            <CardTitle>Admin Notification Feed</CardTitle>
            <p className="text-sm text-muted-foreground">
              Showing latest notifications for your admin account.
            </p>
          </CardHeader>
            ...
            {!notifications || notifications.length === 0 ? (
              <div className="py-12 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Admin notifications about disputes, users, reports, projects,
